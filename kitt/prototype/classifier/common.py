...
class HSigmoid(Layer):
    ...
    def __init__(self, name="HSigmoid", **kwargs):
...
class HSwish(Layer):
    ...
    def __init__(self, name="HSwish", **kwargs):
...
def get_activation_layer(x, activation, name="activ"):
    """
...
def update_keras_shape(x):
    ...
    if not hasattr(x, "_keras_shape"):
        x._keras_shape = tuple(
            [int(d) if (d is not None) and (d != 0) else None for d in x.shape]
        )
...
def batchnorm(x, momentum=0.9, epsilon=1e-5, name=None):
    ...
    x = nn.BatchNormalization(
        axis=get_channel_axis(), momentum=momentum, epsilon=epsilon, name=name
    )(x)
    return x


def maxpool2d(x, pool_size, strides, padding=0, ceil_mode=False, name=None):
    """
    ...
    if K.backend() == "tensorflow":
        ...
        if (padding[0] > 0) or (padding[1] > 0):
            ...
            x = nn.Lambda(
                (
                    lambda z: tf.pad(
                        z, [[0, 0], [0, 0], [padding[0]] * 2, [padding[1]] * 2], mode="REFLECT"
                    )
                )
                if is_channels_first()
                else (
                    lambda z: tf.pad(
                        z, [[0, 0], [padding[0]] * 2, [padding[1]] * 2, [0, 0]], mode="REFLECT"
                    )
                )
            )(x)
        padding_ke = "valid"
    ...
    x = nn.MaxPool2D(
        pool_size=pool_size, strides=strides, padding=padding_ke, name=name + "/pool"
    )(x)
    return x
...
def avgpool2d(x, pool_size, strides, padding=0, ceil_mode=False, name=None):
    ...
    if K.backend() == "tensorflow":
        ...
        x = nn.AvgPool2D(pool_size=pool_size, strides=1, padding="valid", name=name + "/pool")(
            x
        )

        if (strides[0] > 1) or (strides[1] > 1):
            x = nn.AvgPool2D(
                pool_size=1, strides=strides, padding="valid", name=name + "/stride"
            )(x)
        return x
        ...
        if (padding[0] > 0) or (padding[1] > 0):
            ...
            x = nn.Lambda(
                ...
                if is_channels_first()
                    ...
                    lambda z: tf.pad(z, [[0, 0], [padding[0]] * 2, [padding[1]] * 2, [0, 0]])
                )
            )(x)
            if not (
                (padding[0] == padding[1])
                and (kernel_size[0] == kernel_size[1])
                and (kernel_size[0] // 2 == padding[0])
            ):
                extra_pad = True
        padding_ke = "valid"
    else:
...
def conv1x1(x, in_channels, out_channels, strides=1, groups=1, use_bias=False, name="conv1x1"):
    """
...
def conv3x3(x, in_channels, out_channels, strides=1, padding=1, groups=1, name="conv3x3"):
    """
...
def depthwise_conv3x3(x, channels, strides, name="depthwise_conv3x3"):
    """
...
def conv_block(
    ...
    if use_bn:
        x = batchnorm(x=x, epsilon=bn_epsilon, name=name + "/bn")
    if activation is not None:
        x = get_activation_layer(x=x, activation=activation, name=name + "/activ")
    return x
...
def pre_conv1x1_block(
    x, in_channels, out_channels, strides=1, return_preact=False, name="preres_conv1x1"
):
    """
...
def pre_conv3x3_block(
    x, in_channels, out_channels, strides=1, return_preact=False, name="pre_conv3x3_block"
):
    """
...
def channel_shuffle_lambda(channels, groups, **kwargs):
    """
...
def se_block(
    ...
    w = (
        HSigmoid(name=name + "/hsigmoid")(w)
        if approx_sigmoid
        else nn.Activation("sigmoid", name=name + "/sigmoid")(w)
    )
    x = nn.multiply([x, w], name=name + "/mul")
    return x
